...
for n in range(n_states):
    energy_level = [n + 0.5 for x in grid_x]
    shifted_psi = [psi[x][n] + n + 0.5  for x in grid_x]  # vertical shift
    pylab.plot(grid_x, energy_level, "black")
    pylab.plot(grid_x, shifted_psi)
pylab.title('Harmonic oscillator wavefunctions')
pylab.xlabel('$x$', fontsize=16)
pylab.ylabel('$\psi_n(x)$ (shifted)', fontsize=16)
pylab.xlim(-5.0, 5.0)
pylab.ylim(0.0, n_states + 0.5)
