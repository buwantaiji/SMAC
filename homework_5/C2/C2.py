...
for step in range(n_steps):
    ...
    old_weight  =  rho_free(x[knext], x[k], dtau) * \
                   ...
                   math.exp(-dtau * anharmonic(x[k], cubic, quartic))
    new_weight  =  rho_free(x[knext], x_new, dtau) * \
                   rho_free(x_new, x[kprev], dtau) * \
                   math.exp(-dtau * anharmonic(x_new, cubic, quartic))
...
pylab.hist(samples, bins=400, normed=True, label="histogram using PIMC sampling method")

matrixsquaring_data_filename = "pi_x_anharmonic_matrixsquaring_beta_4.0.dat"
...
pylab.title("Probability distribution $\pi(x)$(i.e. the normalized 'single particle density') of a quantum anharmonic oscillator\n"
                + "$\\beta$ = %s\n" % beta
                + "cubic = %s, quartic = %s" % (cubic, quartic))
