...
for step in range(nsteps):
    ...
    k = len(perm_cycle)
    if(k >= cycle_min):
        samples2.append(perm_cycle[0][0])
...
pylab.hist(samples1, normed=True, bins=400, label="samples1", alpha=1.0)
pylab.hist(samples2, normed=True, bins=400, label="samples2", alpha=0.6)
xgrid = [0.1 * i for i in range(-100, 101)]
groundstate_wavefunction_sq = [math.exp(- x ** 2) / math.sqrt(math.pi) for x in xgrid]
pylab.plot(xgrid, groundstate_wavefunction_sq, label="$|\psi_0(x)|^2$", lw=2.5)
pylab.xlabel("x")
pylab.ylabel("$p(x)$")
